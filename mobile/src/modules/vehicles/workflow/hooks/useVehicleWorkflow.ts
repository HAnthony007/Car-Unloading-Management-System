...
export const useVehicleWorkflow = () => {
  ...
  const [vehicleCondition, setVehicleCondition] = useState(inspection?.condition || '');
  const [inspectionNotes, setInspectionNotes] = useState(inspection?.notes || '');
  ...
  useEffect(() => {
    ...
    if (!metadata) {
      setMetadata({
        ...
        arrival: {
          ...
          arrivalDate: new Date(Date.now() - 86400000).toISOString().slice(0,10),
          agent: 'Bolloré',
          origin: 'Anvers',
        },
      });
    ...
    if (!inspection) {
      ...
      setInspectionNotes('Inspection initiale: aucune anomalie visible, carrosserie propre, niveaux OK.');
    }
    if (movements.length === 0) {
      const samples = [
        { from: 'Quai Débarquement', to: 'Zone Tampon', reason: 'Déchargement navire' },
        { from: 'Zone Tampon', to: 'Zone A2', reason: 'Placement initial' },
        { from: 'Zone A2', to: 'Zone B5', reason: 'Optimisation stockage' },
      ];
      samples.forEach(mv => addMovement(mv));
    }
  }, [metadata, inspection, movements.length, setMetadata, setInspection, addMovement]);

  const saveInspection = () => {
    if (vehicleCondition || inspectionNotes) {
      setInspection({ condition: vehicleCondition, notes: inspectionNotes });
    }
  };

  const addMovementRecord = (from: string, to: string, reason: string, cb: () => void) => {
    if (!from || !to) return;
    addMovement({ from, to, reason });
    cb();
  };

  const handleAddNote = (title: string, content: string, reset: () => void) => {
