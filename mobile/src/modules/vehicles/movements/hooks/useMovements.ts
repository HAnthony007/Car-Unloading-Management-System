...
export const useMovements = () => {
  ...
  const [focusedLocationField, setFocusedLocationField] = useState<'from' | 'to' | null>(null);
  ...
  const requestLocation = async () => {
    ...
    try {
      ...
      if (!Location) {
        setCoords({ lat: 14.7167 + (Math.random() - 0.5) * 0.01, lng: -17.4677 + (Math.random() - 0.5) * 0.01 });
        return;
      }
  ...
  const PARKINGS = [
    'Zone Tampon','Zone A1','Zone A2','Zone B3','Zone B5','Quai Débarquement','Parc Export','Inspection Technique','Zone Logistique'
  ];
  ...
  return {
    ...
    coords,
    loadingLoc,
    focusedLocationField, setFocusedLocationField,
    resetForm,
    confirmMovement,
    requestLocation,
    PARKINGS,
